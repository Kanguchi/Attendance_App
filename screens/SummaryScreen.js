...
export default class SummaryScreen extends Component{
  ...
  componentDidMount = async () => {
    ...
    var students_ref = db.ref('/classA').on('value',(data)=>{
  ...
  render() {
    return (
      <View style={{ flex: 1, backgroundColor: '#798FA7'}}>
        <View style={{ flex: 0.1 }}>
        <AppHeader />
        </View>
        <Text style={[styles.title, {marginTop: 70}]}>Present Students List</Text>
        ...
        <View style={{ alignItems: 'center', flex: 1 }}>
            <TouchableOpacity
              ...
              <Text style={{fontSize: 25, color: '#D5DBDB'}}>HomeScreen</Text>
            </TouchableOpacity>
          </View>
      </View>
    );
